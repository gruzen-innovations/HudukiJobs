            ...
            foreach($bdetails as $at){
             ...
             unset($quadetails);
              $qdetailss = userQualificationsDetails::where('employee_auto_id','=',$at->employee_auto_id)->get();
                ...
                $details[] = array("_id"=>$at->_id,"employee_auto_id"=>$at->employee_auto_id,"email_id"=>$email_id,"mobile_number"=>$mobile_number,"resume"=>$at->resume,"video_resume"=>$at->video_resume,"profile_picture"=>$at->profile_picture,"first_name"=>$at->first_name,"middle_name"=>$at->middle_name,
                "last_name"=>$at->last_name,"gender"=>$at->gender,"date_of_birth"=>$at->date_of_birth,"address"=>$at->address,"city"=>$at->city,"pincode"=>$at->pincode,"fresher_or_experienced"=>$at->fresher_or_experienced,"skills"=>$at->skills,"prefered_jobrole"=>$at->prefered_jobrole,"prefered_job_locaion"=>$at->prefered_job_locaion,
                "preferred_job_type"=>$at->preferred_job_type,"preferred_shift"=>$at->preferred_shift,"field_of_experience"=>$at->field_of_experience,"total_year_experience"=>$at->total_year_experience,"employment_type"=>$at->employment_type,"description_of_project"=>$at->description_of_project,"advance_skills"=>$at->advance_skills,"current_ctc"=>$at->current_ctc,"expected_ctc"=>$at->expected_ctc,"Qualifications_data"=>$quadetails,"work_details_data"=>$wfdetails);
            }
            return response()->json([
                'status' => 1, 
