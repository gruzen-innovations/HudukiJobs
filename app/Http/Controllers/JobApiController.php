           ...
           if($edata->isNotEmpty()){
             ...
             unset($quadetails);
              $qdetailss = userQualificationsDetails::where('employee_auto_id','=',$at->employee_auto_id)->get();
	            ...
	            $atdetails[] = array("resume"=>$at->resume,"video_resume"=>$at->video_resume,"profile_picture"=>$at->profile_picture,"first_name"=>$at->first_name,"middle_name"=>$at->middle_name,
	            "last_name"=>$at->last_name,"gender"=>$at->gender,"date_of_birth"=>$at->date_of_birth,"address"=>$at->address,"city"=>$at->city,"pincode"=>$at->pincode,"fresher_or_experienced"=>$at->fresher_or_experienced,
	            "skills"=>$at->skills,"prefered_jobrole"=>$at->prefered_jobrole,"prefered_job_locaion"=>$at->prefered_job_locaion,"field_of_experience"=>$at->field_of_experience,"total_year_experience"=>$at->total_year_experience,"employment_type"=>$at->employment_type,
	            "description_of_project"=>$at->description_of_project,"advance_skills"=>$at->advance_skills,"preferred_job_type"=>$at->preferred_job_type,"preferred_shift"=>$at->preferred_shift,"current_ctc"=>$at->current_ctc,"expected_ctc"=>$at->expected_ctc,"Qualifications_data"=>$quadetails,"work_details_data"=>$wfdetails);
            }
             ...
             $atdatewisetdetails[] = array("employee_auto_id"=>$uts->_id,"name"=>$uts->name,"email_id"=>$uts->email_id,"mobile_number"=>$uts->mobile_number,"status"=>$uts->status,"Register_as"=>$uts->Register_as,
             "profile_photo"=>$uts->profile_photo,"register_date"=>$uts->register_date,"is_bookmark"=>'Yes',"other_details"=>$atdetails);
	         }
	    ...
	    foreach($pcats as $uts){
            ...
            //  }
             ...
             unset($quadetails);
              $qdetailss = userQualificationsDetails::where('employee_auto_id','=',$at->employee_auto_id)->get();
	            ...
	            $atdetailsss[] = array("resume"=>$at->resume,"video_resume"=>$at->video_resume,"profile_picture"=>$at->profile_picture,"first_name"=>$at->first_name,"middle_name"=>$at->middle_name,
	            "last_name"=>$at->last_name,"gender"=>$at->gender,"date_of_birth"=>$at->date_of_birth,"address"=>$at->address,"city"=>$at->city,"pincode"=>$at->pincode,"fresher_or_experienced"=>$at->fresher_or_experienced,
	            "skills"=>$at->skills,"prefered_jobrole"=>$at->prefered_jobrole,"prefered_job_locaion"=>$at->prefered_job_locaion,"preferred_job_type"=>$at->preferred_job_type,"field_of_experience"=>$at->field_of_experience,"total_year_experience"=>$at->total_year_experience,"employment_type"=>$at->employment_type,
	            "description_of_project"=>$at->description_of_project,"advance_skills"=>$at->advance_skills,"current_ctc"=>$at->current_ctc,"expected_ctc"=>$at->expected_ctc,"preferred_shift"=>$at->preferred_shift,"Qualifications_data"=>$quadetails,"work_details_data"=>$wfdetails);
             }
            }else{
                $atdetailsss = array();
            }
           ...
           if($ractions->isNotEmpty()){
            ...
            $jobdetails = Jobs::where('_id','=',$curs->job_auto_id)->where('employer_auto_id','=',$request->get('employer_auto_id'))->get();
             ...
             $atdatewisetdetailss[] = array("job_auto_id"=>$curs->job_auto_id,"job_role"=>$job_role,"employee_auto_id"=>$uts->_id,"name"=>$uts->name,"email_id"=>$uts->email_id,"mobile_number"=>$uts->mobile_number,"status"=>$uts->status,"Register_as"=>$uts->Register_as,
             "profile_photo"=>$uts->profile_photo,"register_date"=>$uts->register_date,"recruiter_action_auto_id"=>$recruiter_action_auto_id,"employer_auto_id"=>$employer_auto_id,"hiring_status"=>$hiring_status,"priority"=>$priority,"rating"=>$rating,"flag"=>$flag,"other_details"=>$atdetailsss);
	         }
            ...
            return response()->json([
                ...
                $pcat = Jobs::where('_id','=',$curs->job_auto_id)->get();
                 if($pcat->isNotEmpty()){
                     ...
                     "min_salary"=>$uts->min_salary,"max_salary"=>$uts->max_salary, "hide_salary"=>$uts->hide_salary,"hiring_process"=>$uts->hiring_process, "walkIn_Interview"=>$uts->walkIn_Interview,"job_option"=>$uts->job_option,"active_status"=>$uts->active_status,
                     "experience_from_years"=>$uts->experience_from_years,"experience_to_years"=>$uts->experience_to_years, "no_of_vacancies"=>$uts->no_of_vacancies,"year_of_passing_from"=>$uts->year_of_passing_from, "year_of_passing_to"=>$uts->year_of_passing_to,"skills"=>$uts->skills,"gender"=>$uts->gender,
                     "percent"=>$uts->percent,"cgpa"=>$uts->cgpa,"job_description"=>$uts->job_description,"recruiter_email"=>$uts->recruiter_email,"recruiter_contact_no"=>$uts->recruiter_contact_no,"walkin_location"=>$uts->walkin_location,"walk_in_time"=>$uts->walk_in_time,"walk_in_date"=>$uts->walk_in_date,
                     "recruiter_action_auto_id"=>$recruiter_action_auto_id,"employer_auto_id"=>$employer_auto_id,"hiring_status"=>$hiring_status,"priority"=>$priority,"rating"=>$rating,"flag"=>$flag,"employer_details"=>$emp_details);
        	         }
   ...
   public function get_employee_list_by_status(Request $request){
	    ...
	    foreach($pcats as $uts){
           ...
           if($ractions->isNotEmpty()){
             ...
             $atdatewisetdetailss[] = array("job_auto_id"=>$curs->job_auto_id,"job_role"=>$job_role,"employee_auto_id"=>$uts->_id,"name"=>$uts->name,"email_id"=>$uts->email_id,"mobile_number"=>$uts->mobile_number,"status"=>$uts->status,"Register_as"=>$uts->Register_as,
             "profile_photo"=>$uts->profile_photo,"register_date"=>$uts->register_date,"recruiter_action_auto_id"=>$recruiter_action_auto_id,"employer_auto_id"=>$employer_auto_id,"hiring_status"=>$hiring_status,"priority"=>$priority,"rating"=>$rating,
             "flag"=>$flag,"other_details"=>$atdetailsss);
	         }
