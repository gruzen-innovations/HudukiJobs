 ...
 $temp = [];
        ...
        if($wjdetails->isNotEmpty()){
            foreach($wjdetails as $slot){
                 if(strtotime($date) <= strtotime($slot->walk_in_date)){
                     $temp[] = $slot;
                 }
                 $timeslotss = $temp;
            }
                ...
                'msg' => config('messages.empty'),
                  ...
                  foreach(collect($timeslotss) as $uts){
                   $emp_details = UserRegister::where('_id','=',$uts->employer_auto_id)->where('Register_as','=','Employer')->get();
                     ...
                     "experience_from_years"=>$uts->experience_from_years,"experience_to_years"=>$uts->experience_to_years,"is_save_job"=>$is_save_job, "no_of_vacancies"=>$uts->no_of_vacancies,"year_of_passing_from"=>$uts->year_of_passing_from, "year_of_passing_to"=>$uts->year_of_passing_to,"skills"=>$uts->skills,"gender"=>$uts->gender,
             ...
             unset($quadetails);
              $qdetailss = userQualificationsDetails::where('employee_auto_id','=',$at->employee_auto_id)->get();
               ...
               $atdetails[] = array("resume"=>$at->resume,"video_resume"=>$at->video_resume,"profile_picture"=>$at->profile_picture,"first_name"=>$at->first_name,"middle_name"=>$at->middle_name,
	            ...
	            "skills"=>$at->skills,"prefered_jobrole"=>$at->prefered_jobrole,"prefered_job_locaion"=>$at->prefered_job_locaion,"field_of_experience"=>$at->field_of_experience,"total_year_experience"=>$at->total_year_experience,"project_count"=>$at->project_count,
	            "description_of_project"=>$at->description_of_project,"advance_skills"=>$at->advance_skills,"current_ctc"=>$at->current_ctc,"expected_ctc"=>$at->expected_ctc,"Qualifications_data"=>$quadetails,"work_details_data"=>$wfdetails);
	            
                $details[] = array("employee_auto_id"=>$at->employee_auto_id,"name"=>$name,"email_id"=>$email_id,"mobile_number"=>$mobile_number,"status"=>$status,"Register_as"=>$Register_as,
             "profile_photo"=>$profile_photo,"register_date"=>$register_date,"is_bookmark"=>'Yes',"other_details"=>$atdetails);
            }
            return response()->json([
                'status' => 1, 
