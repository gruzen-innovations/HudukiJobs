...
class BookingApiController extends Controller
{
    ...
    public function get_candidate_search(Request $request)
    {
        ...
        if($request->get('open_to_work') != ''){
            $emplists = Employee::where('open_to_work', $request->get('open_to_work'))
        ...
        if ($emplists->isEmpty()) {
            ...
            if ($rating === "yes") {
                usort($details, function ($a, $b) {
                    $ratingA = floatval($a['other_details'][0]['average_rating'] ?? 0);
                    $ratingB = floatval($b['other_details'][0]['average_rating'] ?? 0);
